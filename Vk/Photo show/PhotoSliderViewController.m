...
- (void)switchSlide:(BOOL)next prev:(BOOL)prev{
    ...
    [photoView sd_setImageWithURL:[NSURL URLWithString:[data[currentIndex] isKindOfClass:[NSDictionary class]] ? data[currentIndex][@"items"][@"photoBig"] ? data[currentIndex][@"items"][@"photoBig"] : data[currentIndex][@"photo"][@"original"] : data[currentIndex] ] placeholderImage:nil options:SDWebImageRefreshCached progress:^(NSInteger receivedSize, NSInteger expectedSize, NSURL * _Nullable targetURL) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            NSRect popupRect = NSMakeRect(superWindow.frame.origin.x, superWindow.frame.origin.y+superWindow.frame.size.height-imageSize.height, imageSize.width,imageSize.height);
...
//            float ory = superWindow.frame.origin.y+(superWindow.frame.size.height-self.view.window.frame.size.height)/2;
            ...
            if([data[currentIndex] isKindOfClass:[NSDictionary class]]){
    ...
    likesCount.title = [data[currentIndex] isKindOfClass:[NSDictionary class]] ? data[currentIndex][@"items"][@"likesCount"] : @"";
    self.view.window.title = [NSString stringWithFormat:@"%li/%li %@ ",  currentIndex+1, [data count], [data[currentIndex] isKindOfClass:[NSDictionary class]] ? data[0][@"title"] : @""];
}
