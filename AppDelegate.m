...
@synthesize AppsStartSettingsWindowController, superWindowController;
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    AppsStartSettingsWindowController = [secondStory instantiateControllerWithIdentifier:@"AppsStartSettingsWindow"];
    _keyHandle = [[keyHandler alloc]init];
//    [_keyHandle clearAppInfo];
    if(![_keyHandle readAppInfo:nil] && ![YoutubeRWD YoutubeTokensEcxistsInCoreData] && ![TwitterRWD TwitterTokensEcxistsInCoreData] && ![TumblrRWD TumblrTokensEcxistsInCoreData] && ![instaRWD InstagramTokensEcxistsInCoreData]){
 
         [AppsStartSettingsWindowController.window makeKeyAndOrderFront:self];
    
        
    }else{
    
        [superWindowController.window makeKeyAndOrderFront:self];
...
- (NSURL *)applicationDocumentsDirectory {
    // The directory the application uses to store the Core Data store file. This code uses a directory named "home.jojo" in the user's Application Support directory.
    NSURL *appSupportURL = [[[NSFileManager defaultManager] URLsForDirectory:NSApplicationSupportDirectory inDomains:NSUserDomainMask] lastObject];
    return [appSupportURL URLByAppendingPathComponent:@"home.MasterAPI"];
}
- (NSManagedObjectModel *)managedObjectModel {
    ...
    NSURL *modelURL = [[NSBundle mainBundle] URLForResource:@"CoreDataMain" withExtension:@"momd"];
...
- (NSPersistentStoreCoordinator *)persistentStoreCoordinator {
    ...
    if (!shouldFail && !error) {
        ...
        NSURL *url = [applicationDocumentsDirectory URLByAppendingPathComponent:@"OSXCoreDataMainObjC.storedata"];
